...
PARAM_KD_VALUES: npt.NDArray = np.arange(2, 10+1)
PARAM_N_VALUES: npt.NDArray = np.arange(2, 3+1)
PARAM_ALPHA_VALUES: npt.NDArray = np.arange(5, 20+1)
PARAM_DELTA_VALUES: npt.NDArray = np.linspace(0.05, 0.50, 20)
...
def four_bit_multiplier_optimization():
    ...
    def obj_func(solution: npt.NDArray) -> float:
        nonlocal invocation_counter, mixed_set_var
        param_kd: int = int(mixed_set_var.valid_sets[0][int(solution[0])])
        param_n: int = int(mixed_set_var.valid_sets[1][int(solution[1])])
        param_alpha: int = int(mixed_set_var.valid_sets[2][int(solution[2])])
        param_delta: float = float(mixed_set_var.valid_sets[3][int(solution[3])])
        t0: float = time.time()
        # print(f"{invocation_counter=}, {os.getpid()=}, {solution=}")
        # return np.random.random()*10
        accuracy: float = params_to_accuracy(param_kd, param_n, param_alpha, param_delta)
        invocation_counter += 1
        print(f"[iter={invocation_counter+1:d}] [PID={os.getpid()}] [time={(time.time() - t0):.1f}s]: {param_kd=}, {param_n=}, {param_alpha=}, {param_delta=} -> {accuracy=}")
        return accuracy
    ...
    model: DE.JADE = DE.JADE(epoch=DE_EPOCH, pop_size=DE_POP_SIZE)
