...
def parse_dnf_str(input: str, param_kd: float, param_n: float) -> list[SpeciesList]:
    ...
    return parse_dnf(cast(ast.Expr, stmts[0]), param_kd, param_n)

def parse_dnf(tree: ast.Expr, param_kd: float, param_n: float) -> list[SpeciesList]:
    """Convert expression of disjunctive normal form to regulators"""
    match tree.value:
        case ast.BoolOp():
            return parse_top_boolean(tree.value, param_kd, param_n)
        case ast.UnaryOp() | ast.Name():
            return [parse_minterm(tree.value, param_kd, param_n)]
        case _:
...
def parse_minterm(node: ast.BoolOp | ast.UnaryOp | ast.Name, param_kd: float, param_n: float) -> SpeciesList:
    match node:
        case ast.BoolOp():
            ...
            for operand in operands:
                regulators.extend(parse_minterm(operand, param_kd, param_n))
            return regulators
        case ast.UnaryOp():
            ...
            return [{"name": variable.id, "type": -1, "Kd": param_kd, "n": param_n}]
        case ast.Name():
            return [{"name": node.id, "type": 1, "Kd": param_kd, "n": param_n}]
        case _:
            raise Exception(f"Expression isn't a minterm: expected one of {{variable name, boolean operator, unary operator}}, got {node}")

def parse_top_or(top_node: ast.BoolOp, param_kd: float, param_n: float) -> list[SpeciesList]:
    regulators_list: list[SpeciesList] = []
    for node in top_node.values:
        match node:
            case ast.BoolOp() | ast.UnaryOp() | ast.Name():
                regulators_list.append(parse_minterm(node, param_kd, param_n))
            case _:
...
def parse_top_boolean(top_node: ast.BoolOp, param_kd: float, param_n: float) -> list[SpeciesList]:
    match top_node.op:
        case ast.And():
            return [parse_minterm(top_node, param_kd, param_n)]
        case ast.Or():
            return parse_top_or(top_node, param_kd, param_n)
        case _:
...
if __name__ == "__main__":
    ...
    ), param_kd=5, param_n=2)
    print(json.dumps(regulators, indent=4))
    regulators = parse_dnf_str("(A and (not(B))) or (not(A) and B)", param_kd=5, param_n=2)
    print(json.dumps(regulators, indent=4))
